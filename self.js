	...
	.then(files => {
		...
		for (const file of files) {
			try {
				...
				client.err(`Fehler beim Laden von ${file}.js\n${error.stack || error}`);
			}
		}
	});
...
client.once('ready', () => {
	...
	client.log(`(${client.conf.prefix}): [${moment().format('DD.MM.YYY HH:mm:ss')}]:\
	 Stalke ${client.users.size} Nutzer, in ${client.channels.size} Channeln von ${client.guilds.size} Servern.`);
	client.log(`Client took ${client.uptime / 1000}s to login.`);
});

client.on('disconnect', event => {
	client.err(`Disconnected nach \
	${moment.duration(client.uptime).format(' D [Tage], H [Stunden], m [Minuten], s [Sekunden]')}.
