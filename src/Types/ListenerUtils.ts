// straight copy from
// https://github.com/zajrik/yamdbf/blob/master/src/util/ListenerUtil.ts
...
export class ListenerUtil {
	...
	public static registerListeners(emitter: EventEmitter, listenerSource?: object): void {
		...
		for (const listener of Reflect.getMetadata('listeners', listenerTarget.constructor.prototype) as ListenerMetadata[]) {
			...
			emitter[listener.once ? 'once' : 'on'](listener.event,
				(...eventArgs: any[]) => (listenerTarget as any)[listener.method](...eventArgs, ...listener.args));
		}
	}
